...
## 16. user_addresses（用户地址表）

**字段列表：**
- `id` - BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY（主键）
- `user_id` - BIGINT UNSIGNED NOT NULL（用户ID）
- `label` - VARCHAR(20) NOT NULL（标签：家/公司/朋友）
- `consignee_name` - VARCHAR(100) NOT NULL（收货人姓名）
- `consignee_phone` - VARCHAR(20) NOT NULL（收货人电话）
...
- `lng` - DECIMAL(10,6) NULL（经度）
- `lat` - DECIMAL(10,6) NULL（纬度）
...
**外键：**
- `user_addresses_ibfk_1`: user_id → users(id)
...
- **总表数：21 个**
- **主要业务模块：**
  - 用户模块：users, user_referrals, user_addresses
