...
- `merchant_balance` - DECIMAL(14,4) NOT NULL DEFAULT 0.00（商家余额）
...
- `bank_name` - VARCHAR(255) NOT NULL DEFAULT ''（银行名称）
...
- `buy_rule` - TEXT（购买规则）
- `freight` - DECIMAL(12,4) DEFAULT 0.00（运费）
- `created_at` - DATETIME DEFAULT CURRENT_TIMESTAMP（创建时间）
- `updated_at` - DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP（更新时间）
...
- `total_amount` - DECIMAL(12,4) NOT NULL（总金额）
- `original_amount` - DECIMAL(12,4) DEFAULT 0.00（原始金额）
...
- `total_price` - DECIMAL(12,4) NOT NULL（总价）
- `created_at` - DATETIME DEFAULT CURRENT_TIMESTAMP（创建时间）
...
- `balance` - DECIMAL(14,4) NOT NULL DEFAULT 0.00（余额）
- `created_at` - DATETIME DEFAULT CURRENT_TIMESTAMP（创建时间）
...
- `reward_amount` - DECIMAL(12,4) NOT NULL DEFAULT 0.00（奖励金额）
- `created_at` - DATETIME DEFAULT CURRENT_TIMESTAMP（创建时间）
...
- `opening_balance` - DECIMAL(10,4) NOT NULL（期初余额）
- `income` - DECIMAL(10,4) NOT NULL（收入）
- `withdraw` - DECIMAL(10,4) NOT NULL（提现）
- `closing_balance` - DECIMAL(10,4) NOT NULL（期末余额）
